...
impl YTMusic{
        ...
        tokio::spawn({
                ...
                async move {
                    ...
                    if response.status().is_success() {
                        ...
                        let api = get_api_ipc();
                        let msg: IpcMessage = IpcMessage::new(None, "Panel:Music","YT:Load Quik Picks",json!({"songs": tracks}));
        ...
        tokio::spawn({
            ...
            async move {
                ...
                if response.status().is_success() {
                    ...
                    let api = get_api_ipc();
                    let msg: IpcMessage = IpcMessage::new(None,"Panel:Music", "YT:Load Next Songs", json!({"songs": tracks}));
                    let _ = api.emit(msg.clone());
        ...
        tokio::spawn({
            ...
            async move {
                ...
                if response.status().is_success() {
                    ...
                    let api = get_api_ipc();
                    let msg: IpcMessage = IpcMessage::new(None, "Panel:Music", "YT:Load Search", json!({"songs": tracks}));
                    let _ = api.emit(msg.clone());   
                }
            }
        });
    ...
    pub fn status(){
        ...
        let msg: IpcMessage = IpcMessage::new(None, "Panel:Music", "YT:ViewStatus", json!({"status": status}));
        let _ = api.emit(msg.clone());
    }
    ...
    pub fn start(){
        ...
        if cookies_vec.len() == 8{
            ...
            let msg: IpcMessage = IpcMessage::new(None, "Panel:Music", "YT:Set Cookies", serde_json::to_value(&combined).unwrap());
            let _ = api.emit(msg.clone());           
        }else{
