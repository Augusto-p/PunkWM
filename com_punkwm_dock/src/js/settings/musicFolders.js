...
function NewMusicFolder(path) {
  ...
  close.addEventListener("click", () => {
    MusicFolderPop(path);
    MusicFoldersList.removeChild(div);
...
function get_Music_Folder_Color(name) {
  ...
  let PartR = (NameCode.substring(0, n) != "" ? (parseInt(NameCode.substring(0, n)) % 256).toString(16) : "0").padStart(2, "0");
  let PartB = (NameCode.substring(n, 2 * n) != "" ? (parseInt(NameCode.substring(n, 2 * n)) % 256).toString(16) : "0").padStart(2, "0");
  let PartG = (NameCode.substring(2 * n, NameCode.length) != "" ? (parseInt(NameCode.substring(2 * n, NameCode.length)) % 256).toString(16) : "0").padStart(2, "0");
...
async function SelectMusicFolder() {
  ...
  try {
    const selected = await __TAURI_PLUGIN_DIALOG__.open({
                      directory: true,
                      multiple: false,    // Solo una carpeta a la vez
                      title: Lang["UI"]["Music Folders"]["Add Folder"]
                    })
    if (selected !== null) {
      MusicFolderAdd(selected)
...
function loadMusicFolders() {
    MusicFoldersList.innerHTML = "";
    let FoldersLocal = JSON.parse(Storage.getItem("Music-Folders")) ?? [];
...
function MusicFolderAdd(folder) {
  if (folder == ""){
    return
  }
  let FoldersLocal = JSON.parse(Storage.getItem("Music-Folders")) ?? [];
  FoldersLocal.push(folder);
  Storage.setItem("Music-Folders", JSON.stringify(FoldersLocal));
  NewMusicFolder(folder);
  MusicFolder.addFolder(folder);
}

function MusicFolderPop(folder){
  let folderLocal = JSON.parse(Storage.getItem("Music-Folders")) ?? [];
  folderLocal.pop(folder);
  Storage.setItem("Music-Folders", JSON.stringify(folderLocal));
  MusicFolder.popFolder(folder)
}
