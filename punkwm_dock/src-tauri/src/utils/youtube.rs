...
impl YTMusic{
                    ...
                    if response.status().is_success() {
                        ...
                        if let Some(songs) = json["contents"]["singleColumnBrowseResultsRenderer"]["tabs"][0]["tabRenderer"]["content"]["sectionListRenderer"]["contents"][0]["musicCarouselShelfRenderer"]["contents"].as_array(){
                            for item in songs {
                                ...
                                let song = YTSong::new(id,cover,title,album,artist, "YT-Music".to_string(), None,);
                                tracks.push(song);
                            }

                        }
                        let api = get_api_ipc();
                        let msg: IpcFrontMessage = IpcFrontMessage::new("Panel:Music","YT:Load Quik Picks",json!({"songs": tracks}));
                ...
                if response.status().is_success() {
                    ...
                    if let Some(songs) = json["contents"]["singleColumnMusicWatchNextResultsRenderer"]["tabbedRenderer"]["watchNextTabbedResultsRenderer"]["tabs"][0]["tabRenderer"]["content"]["musicQueueRenderer"]["content"]["playlistPanelRenderer"]["contents"].as_array(){
                        for item in songs {
                            ...
                            let song = YTSong::new(id,cover,title,album,artist,"YT-Music".to_string(),Some(duration),);
                            tracks.push(song);
                        }
                    }
                    let api = get_api_ipc();
                    let msg: IpcFrontMessage = IpcFrontMessage::new("Panel:Music", "YT:Load Next Songs", json!({"songs": tracks}));
                    let _ = api.emit(msg.clone());
        ...
        tokio::spawn({
            ...
            async move {
                ...
                if response.status().is_success() {
                    ...
                    let api = get_api_ipc();
                    let msg: IpcFrontMessage = IpcFrontMessage::new("Panel:Music", "YT:Load Search", json!({"songs": tracks}));
                    let _ = api.emit(msg.clone());   
                }
            }
        });
    ...
    pub fn start_song(msg: serde_json::Value){
        ...
        if let Some(window) = api.get_handle().get_webview_window("yt_music_player") {
            window.navigate(url.parse().unwrap()).unwrap();
        }else{   
            let _ = WebviewWindowBuilder::new(&api.get_handle(),"yt_music_player",
    ...
    pub fn status(){
        let api = get_api_ipc();
        let mut status = "OFF";
        if let Some(_window) = api.get_handle().get_webview_window("yt_music_player") {
            status = "ON";
        }
        let msg: IpcFrontMessage = IpcFrontMessage::new("Panel:Music", "YT:ViewStatus", json!({"status": status}));
        let _ = api.emit(msg.clone());
    }
    ...
    pub fn start(){
        let api = get_api_ipc();
        let path = api.get_handle().path().app_data_dir().unwrap().join("cookies");
        ...
        if cookies_vec.len() == 8{
            ...
            let msg: IpcFrontMessage = IpcFrontMessage::new( "Panel:Music", "YT:Set Cookies", serde_json::to_value(&combined).unwrap());
            let _ = api.emit(msg.clone());           
        }else{
            let _ = WebviewWindowBuilder::new(&api.get_handle(),"yt_music_load_data",WebviewUrl::App("ytm.html".into()),)
                .user_agent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36")
                .title("__")
                .visible(false)
                .on_navigation(move |url : &tauri::Url| {
                    ...
                    if url_clon.to_string().contains("about:blank"){
                        if let Some(window) = api.get_handle().get_webview_window("yt_music_load_data") {
                            window.show().unwrap();
                        }                     
                    }
...
impl YTSong{
    pub fn new(id: String, cover: String, title: String, album: String, artist: String, mode: String,duration: Option<String>,)->Self{
        Self{id, cover, title, album,artist,mode,duration}
    }
}