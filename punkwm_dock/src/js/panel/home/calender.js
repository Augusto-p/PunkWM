const diary = document.getElementById("diary-content");

function Load_Diary(events) {
    diary.removeAttribute("data-mode");
    let Diary = {}
    

    events.forEach(event => {
        let date = new Date(event.start_datetime_utc * 1000)
        let DayId = formatLocalISODate(date);        
        if (Object.keys(Diary).indexOf(DayId) == -1) {
            Diary[DayId] = []
        }
        event["start_date"] = {
            "day": capitalize(new Intl.DateTimeFormat(undefined, { weekday: "short" }).format(date)),
            "dayN": date.getDate(),
            "month": capitalize(new Intl.DateTimeFormat(undefined, { month: "short" }).format(date)),
            "hour": new Intl.DateTimeFormat(undefined, {hour: "2-digit",minute: "2-digit",hour12: false}).format(date)
        }
        Diary[DayId].push(event);
        
    });
    diary.innerHTML = ""
    Object.keys(Diary).sort().forEach(key => {
        let day_events = Diary[key];
        let day = day_events[0]["start_date"]["day"];
        let dayN = day_events[0]["start_date"]["dayN"];
        let month = day_events[0]["start_date"]["month"];
        events = ""
        day_events.forEach(event => {
            events +=`<div class="event calendar-colorid-${event.color_id}">
                            <span>${event.summary.trim()}</span>
                            <span>${event.start_date.hour.trim()} ${event.location ? "-": ""} ${event.location ?event.location.trim(): ""}</span>

                        </div>`
            
            
        });
        diary.innerHTML += `<div class="day"><div class="day-view">
                            <span>${day}</span>
...
function set_diary_load_mode() {
    diary.innerHTML = `<span class="loader"></span><span class="loader"></span><span class="loader"></span>`;
    diary.setAttribute("data-mode", "loader");
  
}

function set_diary_login_mode(url){
    diary.innerHTML = `<button onclick="oauth_login('${url}')">
                        <svg xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" xmlns:svg="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 268.1522 273.8827" overflow="hidden" xml:space="preserve">
                            <defs>
                                <linearGradient id="a">
    ...
    diary.setAttribute("data-mode", "login");
}
