...
use crate::input_handler::ShortcutManager;
use crate::utils::google::oauth::Google;
use crate::utils::apps::AppManager;
...
use smithay::{
    ...
    utils::{Clock, Logical, Monotonic, Point, Rectangle, Serial, Time,SERIAL_COUNTER as SCOUNTER,},
    wayland::{
...
use crate::ipc::socket::PunkIPC;
// use crate::utils::layouts::{LayoutsManager,LayoutType};
use crate::workspace::Workspace;
use crate::config::config::GLOBAL_CFG;
use crate::utils::network::device::DeviceState;
...
pub struct AnvilState< BackendData: Backend + 'static> {
    ...
    pub keybindings: ShortcutManager,
    pub punk_ipc: PunkIPC,
    pub battery: Option<Battery>,
    pub network_status : DeviceState,
    pub google: Option<Arc<Mutex<Google>>>,
    pub apps_manager: AppManager,
...
impl<BackendData: Backend> InputMethodHandler for AnvilState<BackendData> {
    ...
    fn parent_geometry(&self, parent: &WlSurface) -> Rectangle<i32, smithay::utils::Logical> {
        self.workspaces[self.current_workspace].space
            .elements()
...
impl<BackendData: Backend> PointerConstraintsHandler for AnvilState<BackendData> {
    ...
    fn cursor_position_hint(
        ...
        if with_pointer_constraint(surface, pointer, |constraint| {
            ...
            let origin = self
                .workspaces[self.current_workspace].space
                .elements()
                .find_map(|window| {
    ...
    fn request_activation(
        ...
        if token_data.timestamp.elapsed().as_secs() < 10 {
            ...
            if let Some(window) = w {
                self.workspaces[self.current_workspace].space.raise_element(&window, true);
            }
        }
    }
}
                        ...
                        with_states(&root, |states| {
                            surface_primary_scanout_output(&root, states).or_else(|| {
                                self.window_for_surface(&root).and_then(|window| {
                                    self.workspaces[self.current_workspace].space.outputs_for_element(&window).first().cloned()
                                })
                            })
                        })
                    } else {
                        self.window_for_surface(&root)
                            .and_then(|window| self.workspaces[self.current_workspace].space.outputs_for_element(&window).first().cloned())
                    }
                })
                .or_else(|| self.workspaces[self.current_workspace].space.outputs().next().cloned());
...
impl<BackendData: Backend + 'static> XWaylandKeyboardGrabHandler for AnvilState<BackendData> {
    fn keyboard_focus_for_xsurface(&self, surface: &WlSurface) -> Option<KeyboardFocusTarget> {
        ...
        Some(KeyboardFocusTarget::Window(elem.window.clone()))
    }
}
...
impl<BackendData: Backend + 'static> AnvilState<BackendData> {
    pub fn init(
        ...
        std::process::Command::new("swaybg").args(&["-i",cfg.styles().get_bg(),"-m","fill",]).spawn().expect("failed to start swaybg");        
        ...
        let google = Google::new().map(|g| Arc::new(Mutex::new(g)));
        ...
        AnvilState {
            ...
            current_workspace: 0,
            punk_ipc: PunkIPC::new(),
            dock: punk_dock,
            battery: None,
            keybindings:ShortcutManager::new(),
            network_status: DeviceState::Disconnected,
            google,
            // space: Space::default(),
    ...
    pub fn switch_workspace(&mut self, workspace: usize) {
        ...
        if target < self.workspaces.len() {
            if self.current_workspace == target {
                return; // No hacer nada si ya estamos en ese workspace
            }

            self.current_workspace = target;
            let seat = self.seat.clone();
            seat.get_keyboard().unwrap().set_focus(self, None, Serial::from(0));
            self.display_handle.flush_clients().ok();
            self.current_workspace_arrange();
            self.layout_set_sender();
            self.workspace_update_sender();
        } else {
            warn!("Intento de cambiar a un workspace inexistente: {}", target);
        }
    }

    pub fn toggle_layout(&mut self) {
        self.current_workspace_mut().next_layout();
        self.current_workspace_arrange();
        self.layout_set_sender();
    }

    pub fn window_closed(&mut self){
        if let Some((index, window)) = self.get_focus_win() {
            self.current_workspace_mut().windows.remove(index);
            window.window.toplevel().unwrap().send_close();
            self.current_workspace_arrange();
        }
    }
    ...
    pub fn focus_next(&mut self){
        ...
        if let Some((i, _)) = self.get_focus_win() {
            let windows = &self.current_workspace().windows;
            if !windows.is_empty() {
                let index = (i + 1) % windows.len();
                if let Some(next_window) = windows.get(index) {
                    let window = next_window.clone();
                    let seat = self.seat.clone();
                    if let Some(keyboard) = seat.get_keyboard() {
                        let focus_target: KeyboardFocusTarget = window.into();
                        keyboard.set_focus(self, Some(focus_target), serial);
                    }
                }
            }
        }
    ...
    pub fn focus_prev(&mut self){
        ...
        if let Some((i, _)) = self.get_focus_win() {
            let windows = &self.current_workspace().windows;
            if !windows.is_empty() {
                let index = (i - 1 + windows.len()) % windows.len();
                if let Some(next_window) = windows.get(index) {
                    let window = next_window.clone();
                    let seat = self.seat.clone();
                    if let Some(keyboard) = seat.get_keyboard() {
    ...
    pub fn move_to_workspace(&mut self, workspace: usize){
        ...
        if target < self.workspaces.len() {
            ...
            if let Some((index, win)) = self.get_focus_win() {
                self.current_workspace_mut().windows.remove(index);
                self.current_workspace_mut().space.unmap_elem(&win);
                self.workspaces[target].windows.push(win);
                let seat = self.seat.clone();
                seat.get_keyboard().unwrap().set_focus(self, None, Serial::from(0));
                self.display_handle.flush_clients().ok();
                self.current_workspace_arrange();
    ...
    pub fn switch_and_move_to_workspace(&mut self, workspace: usize){
        let target = workspace - 1;
        if target < self.workspaces.len() {
            if self.current_workspace == target {
                return; // No hacer nada si ya estamos en ese workspace
    ...
    pub fn window_prev(&mut self){
        let win_len = self.current_workspace().windows.len();
        if win_len <= 1{return;}
        if let Some((i, win)) = self.get_focus_win() {
            let index = (i + win_len - 1) % win_len;
            self.current_workspace_mut().windows.remove(i);
            self.current_workspace_mut().windows.insert(index, win);
            self.current_workspace_arrange();
        }
    }
...
impl<BackendData: Backend + 'static> AnvilState<BackendData> {
    pub fn pre_repaint(&mut self, output: &Output, frame_target: impl Into<Time<Monotonic>>) {
        ...
        self.workspaces[self.current_workspace].space.elements().for_each(|window| {
    ...
    pub fn post_repaint(
        ...
        self.current_workspace().space.elements().for_each(|window| {
            window.with_surfaces(|surface, states| {
                let primary_scanout_output = surface_primary_scanout_output(surface, states);
            ...
            if self.workspaces[self.current_workspace].space.outputs_for_element(window).contains(output) {
...
impl SpaceExt for Space<WindowElement> {
    fn vh(&self) -> i32 {
        self.outputs()
            .next()
            .and_then(|output| output.current_mode())
            .map(|mode| (mode.size.h / 100) as i32)
            .unwrap_or(0)
    }
}
...
impl<BackendData: Backend + 'static> AnvilState<BackendData> {
    ...
    pub fn current_workspace_arrange(&mut self){
        let current = self.current_workspace;
        let workspace = &mut self.workspaces[current];
        let dock = &mut self.dock;
        if dock.is_lock(){
            workspace.arrange_lock_window(dock.lockwindow.clone());
            return;
        }
        workspace.arrange(dock);
    }

    pub fn get_focus_win(&self) -> Option<(usize, WindowElement)> {
        let dock = &self.dock;
        if dock.is_lock() {
            return None; // Corregido: antes hacías `return;` que devolvía `()`, rompiendo la firma.
        }

        let seat = self.seat.clone();
        let keyboard = seat.get_keyboard()?;
        let focused_target = keyboard.current_focus()?;

        // 1. Buscamos el índice de la ventana que coincide con el foco del teclado
        let index = self.current_workspace().windows.iter().position(|window| {window.wl_surface() == focused_target.wl_surface()})?; // Si no encuentra ninguna coincidencia, sale temprano devolviendo None
        // 2. Obtenemos la ventana usando el índice encontrado
        let focused_window = self.current_workspace().windows[index].clone();

        // 2. Ahora que `focused_window` es un `WindowElement`, las comparaciones con el dock funcionan perfectamente
        if Some(focused_window.clone()) != dock.window && Some(focused_window.clone()) != dock.lockwindow {
            Some((index, focused_window))
        } else {
            None
        }
    }
