...
use crate::ipc::message::{IPC_PUNK_DOCK_NAME,PunkIpcMessage,DHClientDataMessage, IpcMessageEncrypted,IpcMessage};
use crate::{
    drawing::*,
    render::*,
    shell::WindowElement,
...
use crate::utils::{battery::Battery,network::device::NetworkDevice};
...
pub fn run_udev() {
    ...
    let (punk_ipc_tx, punk_ipc_rx) = channel::<PunkIpcMessage>();
    event_loop.handle().insert_source(punk_ipc_rx, |event, _, state| {
        match event {
            Event::Msg(msg) => {
                if msg.sender == IPC_PUNK_DOCK_NAME{
                    if msg.type_ == String::from("DOCK CONNECTION D/H"){
                        let data: DHClientDataMessage = serde_json::from_value(msg.data).unwrap();
                        state.punk_ipc.set_p(data.p);
                        state.punk_ipc.set_g(data.g);
                        state.punk_ipc.set_your_pub_key(data.pub_key);
                        state.punk_ipc.generate_pub_key();
                        state.punk_ipc.generate_key();
                        state.punk_ipc.link();
                    }else if msg.type_ == String::from("DOCK MESSAGE ENCODE"){
                        let data: IpcMessageEncrypted = serde_json::from_value(msg.data).unwrap();
                        let message_json = state.punk_ipc.decrypt_message(data);
                        let message:IpcMessage = serde_json::from_str(&message_json).unwrap();
                        state.process_dock_handler(message);
                    }
                }else{
                    println!("mensaje ipc {:?}", msg);
    ...
    */
    ...
    .insert_source(
        ...
        move |_, _, state| {
            ...
            if let Some(capacity) = std::fs::read_to_string(format!("{}/capacity",battery_path),).ok()
                .and_then(|x| {x.trim().parse::<u8>().ok()}){
                if let Some(charging) = std::fs::read_to_string(format!("{}/status",battery_path),).ok()
                    .map(|x| x.trim() == "Charging"){
                    let new_batery = Some(Battery{
                        capacity: capacity,
                        charging: charging
                    });
                    let changed = &state.battery != &new_batery;

                    if changed {
                        state.battery = new_batery;
                        state.battery_update_sender();
                    }
                }
            }
    ...
    */
    ...
    .insert_source(
        ...
        move |_, _, state| {
            ...
            let network = NetworkDevice::get_state();

            // solo si cambió
            if network != state.network_status {

                println!(
                    "network changed: {:?}",
                    network
                );
                state.network_status = network;
            }
     ...
     */
     ...
     let _ = state.punk_ipc.start_ipc_server(punk_ipc_tx);
    ...
    while state.running.load(Ordering::SeqCst) {
        ...
        if result.is_err() {
            ...
            state.workspaces[state.current_workspace].space.refresh();
...
impl AnvilState<UdevData> {
    ...
    fn connector_connected(&mut self, node: DrmNode, connector: connector::Info, crtc: crtc::Handle) {
        ...
        if non_desktop {
            ...
            let x = self
                ...
                .outputs()
                .fold(0, |acc, o| acc + self.workspaces[self.current_workspace].space.output_geometry(o).unwrap().size.w);
            let position = (x, 0).into();

            output.set_preferred(wl_mode);
            output.change_current_state(Some(wl_mode), None, None, Some(position));
            for i in 0..self.workspaces.len(){
                self.workspaces[i].space.map_output(&output, position);
    ...
    fn connector_disconnected(&mut self, node: DrmNode, connector: connector::Info, crtc: crtc::Handle) {
            ...
            .position(|(handle, _)| *handle == connector.handle())
        {
            ...
            self.workspaces[self.current_workspace].space.unmap_output(&surface.output);
            self.workspaces[self.current_workspace].space.refresh();
        }
    ...
    fn device_changed(&mut self, node: DrmNode) {
        ...
        // crate::shell::fixup_positions(&mut self.workspaces[self.current_workspace].space, self.pointer.current_location());
    }
    ...
    fn frame_finish(&mut self, dev_id: DrmNode, crtc: crtc::Handle, metadata: &mut Option<DrmEventMetadata>) {
        ...
        let output = if let Some(output) = self.workspaces[self.current_workspace].space.outputs().find(|o| {
