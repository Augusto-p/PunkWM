...
use crate::ipc::{server::start_ipc_server,
    senders::{  layout::sender_layout_set, battery::sender_battery_update, workspace::sender_workspace_update, 
        ...
        system::{sender_system_load_panel, sender_system_panel_close,sender_system_bg_lockscreen, sender_system_lock_valid},
            ...
            Event::ClientMessage(e) if e.type_ == wake_up_atom => {
                while let Ok(custom) = rx.try_recv() {
                    match custom {
                        ...
                        CustomEvent::DockLock()=>{
                            sender_system_bg_lockscreen(config.styles.lock_bg.clone());
                            sender_system_user_lockscreen(SysUser::get_photo(), SysUser::get_user());

                        },
                        CustomEvent::LockAuth(password)=>{
                            let pass:&str;
                            if password.starts_with('"') && password.ends_with('"') && password.len() >= 2 {
                                pass = &password[1..password.len()-1];
                            }else{
                                pass = &password;
                            }
                            
                            if SysUser::auth(pass.trim()){
                                sender_system_lock_valid();
                                if let Some(ref mut lock) = wm.lock {
                                    lock.close();
                                }
                            }
                        }
                            ...
                            std::thread::spawn(move || {
                                ...
                                rt.block_on(async move {
                                    if let Some(weather) = get_weather(
                                        weather_cfg.get_city(), 
                                        weather_cfg.get_state(), 
                                        weather_cfg.get_country(), 
                                        lang_cfg, 
                                        weather_cfg.get_units()).await{
