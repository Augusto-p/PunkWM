...
impl WindowElement {
    pub fn surface_under(
        ...
        let surface_under = self.window.surface_under(location - offset.to_f64(), window_type);
        let (under, loc) = match self.window.underlying_surface() {
    ...
    pub fn new(window: smithay::desktop::Window, workspace: usize, title:&String, app_id: &String) -> Self {
        Self { window, workspace, title:title.clone(), app_id:app_id.clone() }
    }

    pub fn title(&self)->String{return self.title.clone()}
    pub fn app_id(&self)->String{return self.app_id.clone()}
    pub fn with_surfaces<F>(&self, processor: F)
    where
        F: FnMut(&WlSurface, &WlSurfaceData),
    {
        self.window.with_surfaces(processor);
    }
    ...
    R::TextureId: Clone + Texture + 'static,
{
    ...
    fn render_elements<C: From<Self::RenderElement>>(
        ...
        let window_bbox = SpaceElement::bbox(&self.window);
