...
impl<BackendData: Backend> XwmHandler for AnvilState<BackendData> {
    ...
    fn map_window_request(&mut self, _xwm: XwmId, window: X11Surface) {
        ...
        let app_id = String::from("");
        let window = WindowElement::new(Window::new_x11_window(window), self.current_workspace().id().into(), &title, &app_id);
        self.current_workspace_mut().windows.push(window.clone());
        self.current_workspace_arrange();
        // place_new_window(&mut self.workspaces[self.current_workspace].space, self.pointer.current_location(), &window, true);
        let bbox = self.workspaces[self.current_workspace].space.element_bbox(&window).unwrap();
        let Some(xsurface) = window.window.x11_surface() else {
            unreachable!()
        };
    ...
    fn mapped_override_redirect_window(&mut self, _xwm: XwmId, window: X11Surface) {
        ...
        let app_id = String::from("");
        let window = WindowElement::new(Window::new_x11_window(window), self.current_workspace().id().into(), &title, &app_id);
        self.workspaces[self.current_workspace].space.map_element(window, location, true);
    }
    ...
    fn unmapped_window(&mut self, _xwm: XwmId, window: X11Surface) {
        let maybe = self
            ...
            .elements()
            .find(|e| matches!(e.window.x11_surface(), Some(w) if w == &window))
            .cloned();
        if let Some(elem) = maybe {
            self.workspaces[self.current_workspace].space.unmap_elem(&elem)
        }
    ...
    fn configure_notify(
        ...
        self.workspaces[self.current_workspace].space.map_element(elem, geometry.loc, false);
    ...
    fn unmaximize_request(&mut self, _xwm: XwmId, window: X11Surface) {
            ...
            .and_then(|data| data.restore())
        {
            ...
            self.workspaces[self.current_workspace].space.map_element(elem, old_geo.loc, false);
        }
    }
    ...
    fn fullscreen_request(&mut self, _xwm: XwmId, window: X11Surface) {
        if let Some(elem) = self
            ...
            .find(|e| matches!(e.window.x11_surface(), Some(w) if w == &window))
        {
            let outputs_for_window = self.workspaces[self.current_workspace].space.outputs_for_element(elem);
    ...
    fn unfullscreen_request(&mut self, _xwm: XwmId, window: X11Surface) {
            ...
            .find(|e| matches!(e.window.x11_surface(), Some(w) if w == &window))
        {
            ...
            if let Some(output) = self.workspaces[self.current_workspace].space.outputs().find(|o| {
                o.user_data()
                    .get::<FullscreenSurface>()
                    .and_then(|f| f.get())
                    .map(|w| &w == elem)
                ...
                window.configure(self.workspaces[self.current_workspace].space.element_bbox(elem)).unwrap();
    ...
    fn resize_request(&mut self, _xwm: XwmId, window: X11Surface, _button: u32, edges: X11ResizeEdge) {
        ...
        let loc = self.workspaces[self.current_workspace].space.element_location(element).unwrap();
...
impl<BackendData: Backend> AnvilState<BackendData> {
    pub fn maximize_request_x11(&mut self, window: &X11Surface) {
        ...
        let old_geo = self.workspaces[self.current_workspace].space.element_bbox(&elem).unwrap();
        let outputs_for_window = self.workspaces[self.current_workspace].space.outputs_for_element(&elem);
        let output = outputs_for_window
            .first()
            // The window hasn't been mapped yet, use the primary output instead
        ...
        if let Some(touch) = self.seat.get_touch() {
            if let Some(start_data) = touch.grab_start_data() {
                ...
                if let Some(element) = element {
                    let mut initial_window_location = self.workspaces[self.current_workspace].space.element_location(element).unwrap();
