...
use smithay::wayland::shell::xdg::XdgToplevelSurfaceData; // Necesitas añadir este
use smithay::{
    desktop::{
...
use super::{
    ...
    SurfaceData, WindowElement, fullscreen_output_geometry,
};
...
impl<BackendData: Backend> XdgShellHandler for AnvilState<BackendData> {
    ...
    fn new_toplevel(&mut self, surface: ToplevelSurface) {
        ...
        // No la añadimos a 'windows' todavía...
        
        smithay::wayland::compositor::add_post_commit_hook(&wl_surface, move |state: &mut Self, _, surf| {
            // Accedemos a los datos que Tauri acaba de enviar
            let metadata = smithay::wayland::compositor::with_states(surf, |states| {
                let data = states.data_map.get::<XdgToplevelSurfaceData>().unwrap();
                let guard = data.lock().unwrap();
                (guard.app_id.clone(), guard.title.clone())
            });

            if let (Some(app_id), Some(title)) = metadata {
                let surface_id = surface_handle.wl_surface().id();

                let mut already_in_windows = false;
                let mut workspace_id = 0;
                while !already_in_windows && workspace_id <9 {
                    already_in_windows = state.workspaces[workspace_id].windows.iter().any(|w| {w.window.toplevel().expect("REASON").wl_surface().id() == surface_id});
                    workspace_id +=1;
                }

                // let already_in_windows = state.current_workspace().windows.iter().any(|w| {w.window.toplevel().expect("REASON").wl_surface().id() == surface_id});


                let is_dock = state.dock.window.as_ref().map(|d| d.window.toplevel().expect("REASON").wl_surface().id() == surface_id).unwrap_or(false);
                if already_in_windows || is_dock {return;}
                let window = WindowElement::new(Window::new_wayland_window(surface_handle.clone()), state.current_workspace().id().into(), &title, &app_id);
                if app_id == "com_punkwm_dock_" && title == state.dock.name && state.dock.window == None{ 
                    state.dock.window = Some(window);
                }else if app_id == "com_punkwm_dock_" && title == format!("Lock_{}", state.dock.name) && state.dock.lockwindow == None{
                    state.dock.lockwindow = Some(window);
                }else{
                    state.current_workspace_mut().windows.push(window);
                }
                println!("{}=>{}", app_id,title);
                state.current_workspace_arrange();
                state.workspace_update_sender();
                
                // Si no es la app especial, la añadimos al tiling normal
    ...
    fn resize_request(&mut self, surface: ToplevelSurface, seat: wl_seat::WlSeat, serial: Serial, edges: xdg_toplevel::ResizeEdge,) {
        ...
        let (app_id, title) = smithay::wayland::compositor::with_states(&wl_surface, |states| {
            let data = states.data_map.get::<XdgToplevelSurfaceData>().expect("La superficie no tiene el rol Toplevel");
            let guard = data.lock().unwrap();
            (guard.app_id.clone(), guard.title.clone())
        });
        let app_id = app_id.unwrap_or_else(|| "Desconocido".to_string());
        let title = title.unwrap_or_else(|| "Sin título".to_string());
        if app_id == "com_punkwm_dock_" && title == self.dock.name { return;}

        if self.current_workspace_mut().layout != LayoutType::Floating{
            return;
        }
        ...
        if let Some(touch) = seat.get_touch() {
            if touch.has_grab(serial) {
                ...
                let loc = self.workspaces[self.current_workspace].space.element_location(&window).unwrap();
    ...
    fn fullscreen_request(&mut self, surface: ToplevelSurface, mut wl_output: Option<wl_output::WlOutput>) {
        ...
        let output_geometry = fullscreen_output_geometry(wl_surface, wl_output.as_ref(), &mut self.workspaces[self.current_workspace].space);

        if let Some(geometry) = output_geometry {
            let output = wl_output
                .as_ref()
                .and_then(Output::from_resource)
                .unwrap_or_else(|| self.workspaces[self.current_workspace].space.outputs().next().unwrap().clone());
    ...
    fn maximize_request(&mut self, surface: ToplevelSurface) {
        ...
        let window = self.window_for_surface(surface.wl_surface()).unwrap();
        let outputs_for_window = self.workspaces[self.current_workspace].space.outputs_for_element(&window);
        let output = outputs_for_window
            .first()
            // The window hasn't been mapped yet, use the primary output instead
            .or_else(|| self.workspaces[self.current_workspace].space.outputs().next())
            // Assumes that at least one output exists
            .expect("No outputs found");
        let geometry = self.workspaces[self.current_workspace].space.output_geometry(output).unwrap();

        surface.with_pending_state(|state| {
            state.states.set(xdg_toplevel::State::Maximized);
            state.size = Some(geometry.size);
        });
        self.workspaces[self.current_workspace].space.map_element(window, geometry.loc, true);
   ...
   fn toplevel_destroyed(&mut self, toplevel: ToplevelSurface) {
        self.current_workspace_mut().windows.retain(|window| window.window.toplevel() != Some(&toplevel));
        self.current_workspace_arrange();
        ...
        println!("Una ventana normal ha solicitado cerrarse o se ha destruido.");
...
impl<BackendData: Backend> AnvilState<BackendData> {
    pub fn move_request_xdg(&mut self, surface: &ToplevelSurface, seat: &Seat<Self>, serial: Serial) {
        if self.workspaces[self.current_workspace].layout != LayoutType::Floating{
            return
        }
        ...
        if let Some(touch) = seat.get_touch() {
            if touch.has_grab(serial) {
                ...
                let mut initial_window_location = self.workspaces[self.current_workspace].space.element_location(&window).unwrap();
    ...
    fn unconstrain_popup(&self, popup: &PopupSurface) {
        ...
        let mut outputs_for_window = self.workspaces[self.current_workspace].space.outputs_for_element(&window);
        ...
        for output in outputs_for_window {
            outputs_geo = outputs_geo.merge(self.workspaces[self.current_workspace].space.output_geometry(&output).unwrap());
        }

        let window_geo = self.workspaces[self.current_workspace].space.element_geometry(&window).unwrap();
