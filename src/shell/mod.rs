...
impl<BackendData: Backend> CompositorHandler for AnvilState<BackendData> {
    ...
    fn commit(&mut self, surface: &WlSurface) {
            ...
            if let Some(window) = self.window_for_surface(&root) {
                ...
                if &root == surface {
                    ...
                    if let Some(buffer_offset) = buffer_offset {
                        let current_loc = self.workspaces[self.current_workspace].space.element_location(&window).unwrap();
                        self.workspaces[self.current_workspace].space.map_element(window, current_loc + buffer_offset, false);
                    }
                }
            }
        }
        ...
        ensure_initial_configure(surface, &self.workspaces[self.current_workspace].space, &mut self.popups)
    }
}
...
impl<BackendData: Backend> WlrLayerShellHandler for AnvilState<BackendData> {
    ...
    fn layer_destroyed(&mut self, surface: WlrLayerSurface) {
        if let Some((mut map, layer)) = self.workspaces[self.current_workspace].space.outputs().find_map(|o| {
