...
impl<BackendData: Backend> PointerGrab<AnvilState<BackendData>> for PointerMoveSurfaceGrab<BackendData> {
    fn motion(
        ...
        data.workspaces[data.current_workspace].space
    ...
    fn motion(
        ...
        match &self.window.window.underlying_surface() {
            ...
            WindowSurface::X11(x11) => {
                let location = data.workspaces[data.current_workspace].space.element_location(&self.window).unwrap();
            ...
            match &self.window.window.underlying_surface() {
                WindowSurface::Wayland(xdg) => {
                    ...
                    if self.edges.intersects(ResizeEdge::TOP_LEFT) {
                        ...
                        let mut location = data.workspaces[data.current_workspace].space.element_location(&self.window).unwrap();
                        ...
                        data.workspaces[data.current_workspace].space.map_element(self.window.clone(), location, true);
                    }
