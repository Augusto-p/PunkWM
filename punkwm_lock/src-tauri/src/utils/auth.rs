...
impl UserInfo{
    pub fn new(username: String, comment: String,photo: Option<String>) ->Self{
        Self{username, comment, photo}
    }

    pub fn _get_comment(self)->String{
        self.comment.clone()
    }
    ...
    pub fn _get_photo(self)->Option<String>{
        self.photo.clone()
    }
        ...
        if parts.len() >= 2 && parts[1] == "P" {
            ...
            if let Some((home, comment)) = get_home_and_comment(username, &passwd_content) {
                ...
                if config_path.exists() {
                    ...
                    users.push(UserInfo::new(username.to_string(),comment,photo,));
                }
            }
        }
    }
